...
| TD-2 | Рефакторинг Config и типизация | ⏳ Не начато | - | dataclass, валидация |
| TD-3 | Расширение тестового покрытия | ⏳ Не начато | - | Config, LLMClient, интеграция |
...
- [ ] Тесты для успешной загрузки с полной конфигурацией
- [ ] Тесты для значений по умолчанию
- [ ] Тесты для ConfigError при отсутствии обязательных полей
- [ ] Тесты для валидации типов (int для max_context_messages)
- [ ] Мок .env файла через `monkeypatch`

**3.2 Улучшение тестов LLMClient**
- [ ] Тесты для `get_response()` с разными входными данными
- [ ] Тесты для обработки ошибок API (timeout, invalid key)
- [ ] Тесты для trimming контекста (> 20 сообщений)
- [ ] Тесты для одновременной работы с несколькими пользователями
- [ ] Тесты для `reset_context()` с несуществующим user_id

**3.3 Интеграционные тесты**
- [ ] Создать `tests/integration/test_bot_flow.py`
- [ ] Тест полного цикла: start → message → response → reset
- [ ] Использовать реальный Bot с моком Telegram API
- [ ] Использовать моки для OpenRouter API
- [ ] Добавить pytest marker: `@pytest.mark.integration`

**3.4 Фикстуры и DRY**
- [ ] Создать `tests/conftest.py` с общими фикстурами
- [ ] Вынести создание Config в фикстуру
- [ ] Вынести мок LLMClient в фикстуру
- [ ] Вынести мок Message в фикстуру

**3.5 Coverage минимум**
- [ ] Обновить `Makefile` с `--cov-fail-under=85`
- [ ] Запустить полный coverage report
- [ ] Исправить недостающие тесты

**Проверка соответствия:**
- [ ] Код следует CONVENTIONS.md
- [ ] Процесс согласован с WORKFLOW.md
- [ ] Соответствует VISION.md (простота тестов)

**Тест после итерации:**
- [ ] `make test` проходит с покрытием >= 85%
- [ ] Все edge cases покрыты
- [ ] Интеграционные тесты проходят
- [ ] conftest.py устраняет дублирование

**Критерий завершения:**
- ✅ Покрытие >= 85%
- ✅ Тесты для Config покрывают все сценарии
- ✅ Тесты для LLMClient покрывают edge cases
...
|---------|-----|------|
| Test coverage | 72% | 85%+ |
| Type checking | Нет | mypy strict |
| Ruff rules | 4 | 10+ |
| SOLID нарушения | 3+ | 0 |
| Code duplication | Есть | Минимум |
| Scalability | Низкая | Средняя |
